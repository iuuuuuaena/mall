...
public class FileUploadController extends BaseController {
    ...
    public Object mUpload(@RequestParam("file") MultipartFile img, Model model, HttpSession session) {
        ...
        if (loginUser == null) {
            ...
            if (res == null) {
                ...
                // 如果保存图片失败，就返回图片上传失败
                // 4001
                return ResultUtils.fail(ResponseStatus.IMG_UPLOAD_FAIL.getCode(), ResponseStatus.IMG_UPLOAD_FAIL.getMsg(), ResponseStatus.IMG_UPLOAD_FAIL);
            } else {
                // 否则 返回储存的路径
                Map<String, String> map = new HashMap<>(1);
                map.put("path", "/img/" + res);
     ...
     */
    @PostMapping("/dUpload")
    public Object dUpload(@RequestParam("file") MultipartFile img,HttpServletRequest request) {
        // 从请求中获取用户登录状态
        UserLoginStatus userLoginStatus = getUserLoginStatus(request);
        if (userLoginStatus == null) {
            // 如果用户没有登录状态的话，就返回用户没有登录的json信息
